//
//  Player_Profile_View.swift
//  Git Gud
//
//  Created by Deanna DiPaolo on 11/2/21.
//

import SwiftUI

struct Player_Profile_View: View {
    
    @Environment(\.managedObjectContext) var moc
    @EnvironmentObject var selectedData: DataSelection
    @EnvironmentObject var select: NavigationSelector
    
    @State var coaches: FetchedResults<Coach>
    @State var players:[Player] = []
    @State var teams:[Team] = []
    
    @State var changeProfileImage = false
    @State var openCamera = false
    @State var openPhotoLib = false
    @State private var imageChanged = true
    @State var imageSelected = UIImage()
    @State private var genericBin = Data(count: 1)
    
    var body: some View {
        NavigationView{
            VStack{
                HStack{
                    Button("Title Screen"){
                        self.select.select = "title screen"
                    }
                    Spacer()
                }
                ForEach(players){ player in
                    if player.id == self.selectedData.playerID {
                        Menu {
                            Button("Choose image from photo library") {
                                openPhotoLib = true
                                changeProfileImage = true
                            }
                            Button("Take new picture") {
                                openCamera = true
                                changeProfileImage = true
                            }
                            
                        }
                    label: {
                            Image(uiImage:UIImage(data: player.profileImage ?? genericBin, scale:1.0) ?? UIImage(named: "default_coach_image")!)
                                .resizable()
                                .frame(width: 150, height: 200)
                                .clipShape(Circle())
                                .overlay(Circle().stroke(Color.white, lineWidth: 4))
                                .shadow(radius: 7)
                        }
                        .sheet(isPresented: $openCamera) {
                             ImagePicker(selectedImage: $imageSelected,
                                 sourceType: .camera)
                             .onDisappear(perform: {
                                 player.profileImage = imageSelected.jpegData(compressionQuality: 1.0)
                                 try? self.moc.save()
                                 imageChanged = true
                             })
                         }
                         .sheet(isPresented: $openPhotoLib) {
                             ImagePicker(selectedImage: $imageSelected,
                                 sourceType: .photoLibrary)
                             .onDisappear(perform: {
                                 player.profileImage = imageSelected.jpegData(compressionQuality: 1.0)
                                 try? self.moc.save()
                                 imageChanged = true
                                 })
                        }
                        HStack{
                            Text(player.firstName ?? "")
                            Text(player.lastName ?? "")
                        }
                    }
                }
                Button("View Stats"){
                    self.select.select = ""
                }
                Button("Take a shot"){
                    self.select.select = ""
                }
                Spacer()
            }
            .onAppear(perform: {
                for coach in coaches{
                    if coach.id == self.selectedData.coachID {
                        teams = (coach.teamList)?.allObjects as? [Team] ?? []
                        for team in teams {
                            if team.id == self.selectedData.teamID {
                                players = (team.playerList)?.allObjects as? [Player] ?? []
                            }
                        }
                    }
                }
            })
            .navigationBarBackButtonHidden(true)
            .navigationBarHidden(true)
        }
        .navigationBarBackButtonHidden(true)
        .navigationBarHidden(true)
    }
}

